...
const ANTIGRAVITY_SAFETY_SETTINGS = DEFAULT_SAFETY_SETTINGS.filter(
...
// (DEFAULT_SAFETY_SETTINGS); Antigravity must match for parity while omitting
// CIVIC_INTEGRITY, which Cloud Code rejects for Antigravity streamGenerateContent.
...
test("transformRequest strips Cloud Code-unsupported CIVIC_INTEGRITY safety setting", async () => {
  const executor = new AntigravityExecutor();
  const callerSafety = [
  ...
  const result = await executor.transformRequest("antigravity/gemini-2.5-flash", body, true, {
    projectId: "project-1",
  });

  if (result instanceof Response) throw new Error("Unexpected Response from transformRequest");
  const innerRequest = result.request as Record<string, unknown>;
  assert.deepEqual(innerRequest.safetySettings, [callerSafety[0]]);
});
