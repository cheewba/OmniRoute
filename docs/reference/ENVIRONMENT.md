...
| `OMNIROUTE_REQUIRE_STORAGE_ENCRYPTION` | `false`              | `src/lib/env/runtimeEnv.ts`                           | When `true`, startup fails unless `STORAGE_ENCRYPTION_KEY` is present and valid. Recommended for non-development deployments.     |
...
| `OMNIROUTE_ENABLE_CLOUD_SYNC`           | `false`                                                         | `src/lib/cloudSync.ts`, `src/instrumentation-node.ts` | Enables OmniRoute cloud-sync/background cloud features. Leave `false` for self-hosted-only deployments.                                                                                                                                                                                           |
| `OMNIROUTE_ENABLE_CLOUDFLARED`          | `false`                                                         | `src/app/api/tunnels/cloudflared/route.ts`            | Enables managed Cloudflare Quick Tunnel support. Leave `false` unless explicitly opting in to managed tunnel creation.                                                                                                                                                                            |
